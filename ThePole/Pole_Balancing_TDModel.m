...
Policy = rand(divisionCart, divisionvCart, divisionPole, divisionvPole, divisionForce); % Pole vPole Cart vCart Force
Policy(1,:,:,:,:) = 0;
Policy(37,:,:,:,:) = 0;
Policy(:,:,1,:,:) = 0;
Policy(:,:,21,:,:) = 0;
...
for generation = 1 : N_Generation
    for g = 1 : N_Gene
        ...
        Start_Pole_Angle = 0.1;%(1/2*pi) * rand(1) - (1/4*pi);  % 시작할 때  Pole의 중앙선으로부터의 각도. in rad
        Start_Cart_Position = 0;%10*rand(1)-5; % 시작할 때 Cart의 중앙선으로부터의 위치. in m
